...
@export var heal_color : Color = Color.FOREST_GREEN

# Called when the node enters the scene tree for the first time.
func _ready():
	SignalBus.connect("on_health_changed", on_sign_health_changed)

# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(delta):
	pass

func on_sign_health_changed(node : Node, amount_changed : int):
